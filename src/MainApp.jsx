...
function MainApp() {
  ...
  // 🔹 función de conexión (permite reconectar)
  const connect = (id) => {
    ...
    socket.onclose = (event) => {
      ...
      if (event.reason === "Mesa cerrada por camarero") {
        ...
        setLastAction("🔒 Mesa cerrada por el servicio");
        setUuid(null);
      } else if (event.reason === "Mesa cerrada por falta de camarero") {
        setClosedByWaiter(true);
        setLastAction("🔒 Mesa cerrada por falta de camarero");
        setUuid(null);
      } else if (event.reason === "Mesa no existe o fue cerrada") {
        setClosedByWaiter(true);
        setLastAction("⚠️ Mesa no existe o fue cerrada");
        setUuid(null);
      } else {
        setLastAction("⚠️ Desconectado, reintentando...");
        // 🔄 reconexión en 5s
        setTimeout(() => connect(id), 5000);
      }
    };
  ...
  const renderIcon = (action, status) => {
    ...
    return (
      <div className={`last-action ${status}`}>
        ...
        <p style={{ color }}>{label} — {labelStatus}</p>
      </div>
    );
  };
